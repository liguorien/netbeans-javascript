 ...
 * @author Nicolas Désy
 */
public class JSFormatSupport  extends ExtFormatSupport {
    
    private TokenContextPath tokenContextPath;
    
    public JSFormatSupport(FormatWriter formatWriter) {
        this(formatWriter, JSTokenContext.contextPath);
    }
    
    public JSFormatSupport(FormatWriter formatWriter, TokenContextPath tokenContextPath) {
        super(formatWriter);
    ...
    public boolean isComment(TokenItem token, int offset) {
        ...
        return (token.getTokenContextPath() == tokenContextPath
                && (tokenID == JSTokenContext.LINE_COMMENT
                || tokenID == JSTokenContext.BLOCK_COMMENT));
    }
    
    public boolean isMultiLineComment(TokenItem token) {
        return (token.getTokenID() == JSTokenContext.BLOCK_COMMENT);
    }
    ...
    public boolean canModifyWhitespace(TokenItem inToken) {
        if (inToken.getTokenContextPath() == JSTokenContext.contextPath) {
     ...
     * getPreviousToken() so it ignores the initial token.
        ...
        while (t != null) {
            if (t.getTokenContextPath() == tokenContextPath) {
                ...
                switch (t.getTokenID().getNumericID()) {
                    case JSTokenContext.SEMICOLON_ID:
                    ...
                    case JSTokenContext.RBRACE_ID:
                    case JSTokenContext.ELSE_ID:
                        return (lit != null) ? lit : t;
                        
                    case JSTokenContext.COLON_ID:
                        TokenItem tt = findAnyToken(t, null, new TokenID[] {JSTokenContext.CASE, JSTokenContext.DEFAULT, JSTokenContext.FOR}, t.getTokenContextPath(), true);
                        if (tt != null) {
                            switch (tt.getTokenID().getNumericID()) {
                                case JSTokenContext.CASE_ID:
                                case JSTokenContext.DEFAULT_ID:
                                case JSTokenContext.FOR_ID:
                                    return (lit != null) ? lit : t;
                            }
                        }
                        break;
                        
                    case JSTokenContext.DO_ID:
                    case JSTokenContext.SWITCH_ID:
                    case JSTokenContext.CASE_ID:
                    case JSTokenContext.DEFAULT_ID:
                        return t;
                        
                    case JSTokenContext.FOR_ID:
                    case JSTokenContext.IF_ID:
                    case JSTokenContext.WHILE_ID:
                        /* Try to find the statement after ( ... )
                         * If it exists, then the first important
                         * token after it is the stmt start. Otherwise
                         * it's this token.
                         */
                        if (lit != null && lit.getTokenID() == JSTokenContext.LPAREN) {
                            // Find matching right paren in fwd dir
                            TokenItem mt = findMatchingToken(lit, token,
                                    JSTokenContext.RPAREN, false);
    ...
    public TokenItem findIf(TokenItem elseToken) {
        if (elseToken == null || !tokenEquals(elseToken,
                JSTokenContext.ELSE, tokenContextPath)
                ) {
    ...
    public TokenItem findSwitch(TokenItem caseToken) {
        if (caseToken == null ||
                (!tokenEquals(caseToken, JSTokenContext.CASE,
                tokenContextPath)
                && !tokenEquals(caseToken, JSTokenContext.DEFAULT,
                tokenContextPath))
                ) {
    ...
    public TokenItem findTry(TokenItem catchToken) {
        if (catchToken == null ||
                (!tokenEquals(catchToken, JSTokenContext.CATCH,
                tokenContextPath))
                ) {
     ...
     *  backward using findStatement() so it ignores
     *  the given token.
    ...
    public TokenItem findStatementStart(TokenItem token) {
        ...
        if (t != null) {
            switch (t.getTokenID().getNumericID()) {
                case JSTokenContext.SEMICOLON_ID: // ';' found
                    ...
                    switch (scss.getTokenID().getNumericID()) {
                        case JSTokenContext.LBRACE_ID: // '{' then ';'
                        case JSTokenContext.RBRACE_ID: // '}' then ';'
                        case JSTokenContext.COLON_ID: // ':' then ';'
                        case JSTokenContext.CASE_ID: // 'case' then ';'
                        case JSTokenContext.DEFAULT_ID:
                        case JSTokenContext.SEMICOLON_ID: // ';' then ';'
                            return t; // return ';'
                        ...
                        case JSTokenContext.ELSE_ID: // 'else' then ';'
                        ...
                        default: // something usual then ';'
                            ...
                            if (bscss != null) {
                                switch (bscss.getTokenID().getNumericID()) {
                                    case JSTokenContext.SEMICOLON_ID: // ';' then stmt ending with ';'
                                    case JSTokenContext.LBRACE_ID:
                                    case JSTokenContext.RBRACE_ID:
                                    case JSTokenContext.COLON_ID:
                                        return scss; //
                ...
                case JSTokenContext.LBRACE_ID: // '{' found
                    return token; // return original token
                    
                case JSTokenContext.RBRACE_ID: // '}' found
                    TokenItem lb = findMatchingToken(t, null,
                            JSTokenContext.LBRACE, true);
                    if (lb != null) { // valid matching left-brace
                        ...
                        if (lbss != null) {
                            switch (lbss.getTokenID().getNumericID()) {
                                case JSTokenContext.ELSE_ID: // 'else {'
                                    // Find the corresponding 'if'
                                ...
                                case JSTokenContext.CATCH_ID: // 'catch (...) {'
                            ...
                            if (lbss.getTokenID().getNumericID() == JSTokenContext.LBRACE_ID) {
    ...
    public int findIndent(TokenItem token) {
        ...
        if (indent < 0) { // if not yet resolved
            ...
            if (t != null) { // valid important token
                switch (t.getTokenID().getNumericID()) {
                    case JSTokenContext.SEMICOLON_ID: // semicolon found
                    ...
                    case JSTokenContext.RPAREN_ID:
                        // Try to find the matching left paren
                        TokenItem rpmt = findMatchingToken(t, null, JSTokenContext.LPAREN, true);
                        if (rpmt != null) {
                    ...
                    case JSTokenContext.COLON_ID:
                        TokenItem ttt = findAnyToken(t, null, new TokenID[] {JSTokenContext.CASE, JSTokenContext.DEFAULT, JSTokenContext.FOR}, t.getTokenContextPath(), true);
                        if (ttt != null && ttt.getTokenID().getNumericID() == JSTokenContext.QUESTION_ID) {
                    ...
                    case JSTokenContext.QUESTION_ID:
                    case JSTokenContext.DO_ID:
                    case JSTokenContext.ELSE_ID:
                        indent = getTokenIndent(t) + getShiftWidth();
                        break;
    ...
    public boolean isForLoopSemicolon(TokenItem token) {
        if (token == null || !tokenEquals(token,
                JSTokenContext.SEMICOLON, tokenContextPath)
                ) {
        ...
        while (token != null) {
            ...
            } else if (tokenEquals(token, JSTokenContext.RPAREN, tokenContextPath)) {
                parDepth++;
                
            } else if (tokenEquals(token, JSTokenContext.LBRACE, tokenContextPath)) {
                if (braceDepth == 0) { // unclosed left brace
                    return false;
                }
                braceDepth--;
                
            } else if (tokenEquals(token, JSTokenContext.RBRACE, tokenContextPath)) {
                braceDepth++;
                
            } else if (tokenEquals(token, JSTokenContext.SEMICOLON, tokenContextPath)) {
    ...
    public boolean getFormatSpaceBeforeParenthesis() {
        ...
        return getSettingBoolean(JavaSettingsNames.JAVA_FORMAT_SPACE_BEFORE_PARENTHESIS,
                                 JSSettingsDefaults.defaultJavaFormatSpaceBeforeParenthesis);
         */
        return false;
    }
    ...
    public boolean getFormatSpaceAfterComma() {
        ...
        /*return getSettingBoolean(JavaSettingsNames.JAVA_FORMAT_SPACE_AFTER_COMMA,
                JSSettingsDefaults.defaultJavaFormatSpaceAfterComma);
         */
    }
    ...
    public boolean getFormatNewlineBeforeBrace() {
        ...
        return getSettingBoolean(JavaSettingsNames.JAVA_FORMAT_NEWLINE_BEFORE_BRACE,
                JSSettingsDefaults.defaultJavaFormatNewlineBeforeBrace);
         */
    }
    ...
    public FormatTokenPosition findLineFirstNonWhitespaceAndNonLeftBrace(FormatTokenPosition pos) {
        ...
        TokenItem rbmt = findMatchingToken(ftp.getToken(),
                eolp != null ? eolp.getToken() : null, JSTokenContext.RBRACE, false);
        if (rbmt != null)
            return ftp;
